...
/**
 * DB 폴백 라우팅(파사드 위임)만 검증한다. Redis 실시간 조회 로직 자체는
 * {@link com.dbidding.auction.service.redis.RedisAuctionQueryServiceTest}가 검증한다.
 */
...
class AuctionQueryServiceTest {
    ...
    /** {@code redisAuctionQueryService} 필드에 실제 {@link RedisAuctionQueryService}를 꽂아 폴백 배선을 검증한다. */
    private void wireRedis(RedisAuctionRealtimeStateReader reader, RedisAuctionStateSeeder stateSeeder) {
        RedisAuctionQueryService redisAuctionQueryService = new RedisAuctionQueryService(
                walletService, cursorCodec, reader, stateSeeder
        );
        ReflectionTestUtils.setField(auctionQueryService, "redisAuctionQueryService", redisAuctionQueryService);
    }
